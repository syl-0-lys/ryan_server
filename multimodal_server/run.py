...
from multimodal_server.handlers import TextModelHandler, MultimodalModelHandler, DeepSeekVLHandler
...
def main():
    ...
    if model_type == ModelType.MULTIMODAL:
        ...
        if model_type_str == "multi_modality":
            print("  → 使用 DeepSeekVLHandler (专用)")
            handler = DeepSeekVLHandler(args.model_path, args.device)
        else:
